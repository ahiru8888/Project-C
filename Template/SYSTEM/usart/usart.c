/**	
 * |----------------------------------------------------------------------
 * | 	Pins_PA9PA10,	//USART1		//F4 DISCO loi phan cung
			Pins_PB6PB7,	////USART1
			Pins_PA2PA3,	//USART2
			Pins_PD5PD6,	//USART2
...
#include "string.h"
#include <stdlib.h>
char pwmString[20]; char getString[20];
extern int pwm;
int x=0; 
/**
 * Internal USART struct
 */
typedef struct KhaiBaoCauTruc{
	int8_t *Buffer;
	uint16_t Size;
	uint16_t Num;
	uint16_t In;
...
void USARTx_Init(USART_TypeDef* USARTx, USART_PinsPack_t pinspack, uint32_t baudrate) {
	...
	if (USARTx == USART6) {
		...
		USART_InitStruct.USART_HardwareFlowControl = USART6_HARDWARE_FLOW_CONTROL;
		USART_InitStruct.USART_Mode = USART6_MODE;
		USART_InitStruct.USART_Parity = USART6_PARITY;
		USART_InitStruct.USART_StopBits = USART6_STOP_BITS;
		USART_InitStruct.USART_WordLength = USART6_WORD_LENGTH;
	}
#endif
#ifdef USE_UART7
	if (USARTx == UART7) {
...
//			getString[x] = USART_Getc(USART3);
//			returnString[x] = getString[x];
//			x++;
//			if(USART_FindCharacter(USART3,'-')) 
//			{
//				memset(returnString, '\0', 20);
//				x=0;
//			}
//			flagduy=1;		
//		#endif
...
void USART3_IRQHandler(void) {
	...
	if (USART_GetITStatus(USART3, USART_IT_RXNE)) {
			...
			getString[x] = USART_ReceiveData(USART3);
			x++;
			if(USART_ReceiveData(USART3)=='e')
			{
				strcpy(pwmString,strtok(getString,","));
				pwm = atoi(pwmString);
				memset(getString, '\0', 20);
				memset(pwmString, '\0', 20);
				x=0;
			}
	}
}
...
void USART6_IRQHandler(void) {
	...
	if (USART_GetITStatus(USART6, USART_IT_RXNE)) {
		...
		#else
			//Put received data into internal buffer
			USART_INT_InsertToBuffer(&F4_USART6, USART6->DR);
		#endif
	}
}
#endif
